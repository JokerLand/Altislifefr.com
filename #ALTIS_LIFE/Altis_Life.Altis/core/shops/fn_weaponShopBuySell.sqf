...
if((life_inv_cb == 0) && (life_cash < _price)) exitWith {hint "Vous n'avez pas assez de liquide et pas de carte bleue !"};

if((_itemInfo select 6) != "CfgVehicles") then
{
    if((_itemInfo select 4) in [4096,131072]) then
    {
        if(!(player canAdd _item) && (uiNamespace getVariable["Weapon_Shop_Filter",0]) != 1) exitWith {_bad = "Vous n'avez pas assez de place pour cet objet."};
    };
};
...
if((_gangbank >= _prixcb) && (life_inv_cb > 0)) then {
    ...
    "Mon argent"
        ...
        if(_action) then {
                ...
                if(_prixcb > life_atmcash) exitWith {hint "Vous n'avez pas assez d'argent sur votre compte !"};
                life_atmcash = life_atmcash - _prixcb;
        ...
        if((life_inv_cb == 0)) then {
                if(_price > life_cash) exitWith {hint "Vous n'avez pas assez d'argent sur vous et pas de carte bleue !"};
                life_cash = life_cash - _price;
                hint parseText format["Vous avez acheté un %1 pour <t color='#8cff9b'>%2$</t>",_itemInfo select 1,[_price] call life_fnc_numberText];
                [_item,true] spawn life_fnc_handleItem;
        } else {
