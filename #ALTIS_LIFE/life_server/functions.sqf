...
private[""_msg"",""_to""];
	...
	_msg = ctrlText 3003;
	_to = ""SAMU"";
	if(_msg == """") exitWith {hint ""Vous devez entrer un message à envoyer !"";ctrlShow[3022,true];};
...
compileFinal "
	...
	ctrlShow[3015,false];
	_msg = ctrlText 3003;
	if(lbCurSel 3004 == -1) exitWith {hint ""Vous devez sélectionner à qui envoyer le message !""; ctrlShow[3015,true];};
	_to = call compile format[""%1"",(lbData[3004,(lbCurSel 3004)])];
	if(isNull _to) exitWith {ctrlShow[3015,true];};
	if(isNil ""_to"") exitWith {ctrlShow[3015,true];};
	if(_msg == """") exitWith {hint ""Vous devez entrer un message à envoyer !"";ctrlShow[3015,true];};
	
	[[_msg,name player,0],""TON_fnc_clientMessage"",_to,false] spawn life_fnc_MP;
	[] call life_fnc_cellphone;
	hint format[""Vous avez envoyé à %1 ce message: %2"",name _to,_msg];
	ctrlShow[3015,true];
";
//To All Cops
...
compileFinal "
	...
	_msg = ctrlText 3003;
	_to = ""Police Secours"";
	if(_msg == """") exitWith {hint ""Vous devez entrer un message à envoyer !"";ctrlShow[3016,true];};
		
	[[_msg,name player,1],""TON_fnc_clientMessage"",true,false] spawn life_fnc_MP;
	[] call life_fnc_cellphone;
	hint format[""Vous avez envoyé à %1 ce message: %2"",_to,_msg];
	ctrlShow[3016,true];
";
//To All Admins
...
compileFinal "
	...
	_msg = ctrlText 3003;
	_to = ""Les Admins"";
	if(_msg == """") exitWith {hint ""Vous devez entrer un message à envoyer !"";ctrlShow[3017,true];};
...
compileFinal "
	...
	if(_msg == """") exitWith {hint ""Vous devez entrer un message à envoyer !"";};
