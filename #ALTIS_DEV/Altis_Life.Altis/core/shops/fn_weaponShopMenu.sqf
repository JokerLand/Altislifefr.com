...
if(!(EQUAL(_level,-1))) then {
    _flag = switch(playerSide) do {case west: {(_level <= (FETCH_CONST(life_copLevel)))}; case east: {(_level <= (FETCH_CONST(life_adaclevel)))}; case independent: {(_level <= (FETCH_CONST(life_medicLevel)))}; default {true};};
