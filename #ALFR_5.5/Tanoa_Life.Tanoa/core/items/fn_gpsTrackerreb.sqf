...
if(!(_unit isKindOf "AllVehicles")) exitWith {hint "Vous ne pouvez pas utiliser le traceur de véhicules sur ceci."};
if(player distance _unit > 7) exitWith {hint "Approchez-vous du véhicule!"};
if(!([false,"gpsTrackerreb",1] call life_fnc_handleInv)) exitWith {};
closeDialog 0;
...
titleText["Vous avez fixé un traceur de véhicules à ce véhicule.","PLAIN"];
...
[_unit] spawn {
	...
	_marker setMarkerTextLocal "Traceur sur "+getText(configFile >> "CfgVehicles" >> typeof _veh >> "displayName");
