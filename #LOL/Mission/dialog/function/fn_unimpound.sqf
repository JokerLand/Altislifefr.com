...
if(typeName life_garage_sp == "ARRAY") then {
	...
	if(life_garage_sp in ["medic_spawn_1","medic_spawn_2","medic_spawn_3"]) then {
		...
		if(life_garage_sp == "pn_ship_spawn") then {
			_pos = getMarkerPos life_garage_sp;
			_posZ = [_pos select 0,_pos select 1,((_pos select 2) + 4)];
			[[_vid,_pid,_posZ,_unit,_price,markerDir life_garage_sp],"TON_fnc_spawnVehicle",false,false] spawn life_fnc_MP;
		} else {
