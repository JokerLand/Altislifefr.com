...
cleanNearItems = {	
	if (_pname != "__SERVER__") then {
		...
		if (!(isNull _player)) then {
			...
			{
				deleteVehicle _x
			} forEach nearestObjects [_player, ["WeaponHolderSimulated"], 2];
			sleep 1;
			{
				deleteVehicle _x
			} forEach nearestObjects [_player, ["GroundWeaponHolder"], 2];
			sleep 1; // Wait
			{
				deleteVehicle _x
			} forEach nearestObjects [_player, ["GroundWeaponHolder"], 2];
...
[32,format["Joueur %1 déconnecté du serveur",_pname]] call life_fnc_logToServer;