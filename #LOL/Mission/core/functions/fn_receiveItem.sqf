...
_diff = [_item,_val,life_carryWeight,life_maxWeight] call life_fnc_calWeightDiff;
...
if(_diff != (_val)) then
{
	if(([true,_item,_diff] call life_fnc_handleInv)) then
	{
		...
		[[_from,_item,str(_val - _diff),_unit],"life_fnc_giveDiff",_from,false] spawn life_fnc_MP;
	}
		else
	{
