...
[] spawn
{
	...
	while{true} do
	{
		...
		_bp = backpack player;
		_state = endu_state;
		_cfg = getNumber(configFile >> "CfgVehicles" >> (backpack player) >> "canBeAssembled");
		_load = round(_cfg / 8);
		if(_state == 4) then {_load = round(_load * 1.3)};
		life_maxWeight = life_maxWeightT + _load;
		waitUntil {backpack player != _bp OR endu_state != _state};
...
[] spawn
{
	while {true} do
	{
		...
		if(life_inv_uranium2 != 0) then {
			...
			player setFatigue 1;
			hintSilent "L'Uranium Brut t'encombre, tu ne peux plus courir !";
		} else {
			if(isForcedWalk player) then {
				player forceWalk false;
			};
...
[] spawn
{
	while {true} do
	{
		...
		while {((player distance (getMarkerPos "Warm_Marker") < 250) && (player getVariable["Revive",TRUE]))} do
		{
			...
			{
				hintSilent "!!! ZONE RADIOACTIVE !!!";
				sleep 5;
			}else
			{
				hintSilent "!!! ZONE RADIOACTIVE !!!\n\n\n VOUS DEVEZ PORTER UNE COMBINAISON";
