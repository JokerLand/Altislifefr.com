...
switch(_type) do
{
	...
	case 1:
	{
		...
		_msg = [_data select 2,24] call KRON_StrLeft;
		_from = [_data select 3,15] call KRON_StrLeft;
		_rowData = [_data select 0, _data select 1, _data select 2, _data select 3];
		_cMessageList lnbAddRow[_from,format["  %1...",_msg]];
	...
	case 4:
	{
		...
		ctrlSetText[88886, format["Message à : %1",life_smartphoneTarget getVariable["realname", name life_smartphoneTarget]]];
