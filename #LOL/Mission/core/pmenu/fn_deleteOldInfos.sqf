...
_result = ["Êtes-vous sûr de vouloir supprimé ce casier judiciaire ?", "Confirmation", true, true] call BIS_fnc_guiMessage;
if(_result) then {
	_pname = player getVariable ["realname",name player];
	_uname = _unit getVariable ["realname",name _unit];
	_message = format["%1 a supprimé le casier judiciaire de %2 !",_pname,_uname];
	adminLog = _message;
	publicVariableServer "adminLog";
	
	[[0,getPlayerUID _unit],"TON_fnc_delInfos",false,false] spawn life_fnc_MP;
	sleep 2;
