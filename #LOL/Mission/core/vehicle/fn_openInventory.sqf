...
if(isNull _vehicle OR !(_vehicle isKindOf "Car" OR _vehicle isKindOf "Air" OR _vehicle isKindOf "Ship" OR _vehicle isKindOf "House_F" OR ((typeOf cursorTarget) IN life_containers_classnames))) exitWith {}; //Either a null or invalid vehicle type.
...
if(_vehicle isKindOf "House_F" OR ((typeOf cursorTarget) IN life_containers_classnames)) then {
...
if(_vehicle isKindOf "House_F" && {count (_vehicle getVariable ["containers",[]]) == 0} && !((typeOf _vehicle) IN life_containers_classnames)) exitWith {hint localize "STR_MISC_NoStorageWarn"; closeDialog 0; _vehicle setVariable["trunk_in_use",false,true];};
if(_veh_data select 0 == -1 && {(!(_vehicle isKindOf "House_F")) && !((typeOf cursorTarget) IN life_containers_classnames)}) exitWith {closeDialog 0; _vehicle setVariable["trunk_in_use",false,true]; hint localize "STR_MISC_NoStorageVeh";};
...
_vehicle spawn
{
	...
	if(_this isKindOf "House_F" && !((typeOf _this) IN life_containers_classnames)) then {
		[[_this],"TON_fnc_updateHouseTrunk",false,false] spawn life_fnc_MP;
	};
	if ((typeOf _this) IN life_containers_classnames) then {
