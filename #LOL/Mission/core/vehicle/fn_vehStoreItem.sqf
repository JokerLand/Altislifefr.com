...
if(life_trunk_vehicle isKindOf "House_F" OR ((typeOf life_trunk_vehicle) IN life_containers_classnames)) then {
	_mWeight = 0;
	if((typeOf life_trunk_vehicle) IN life_containers_classnames) then {
		// Company storage
		_mWeight = 10000;
	} else {
...
if(_ctrl == "goldbar" && {!(life_trunk_vehicle isKindOf "LandVehicle" OR life_trunk_vehicle isKindOf "House_F")}) exitWith {hint "Tu ne peux pas transporter l'or en hélicoptère !"};
if(_ctrl == "uranium1" && {(life_trunk_vehicle isKindOf "AIR")}) exitWith {hint "Tu ne peux pas transporter les déchets d'Uranium en hélicoptère !"};
