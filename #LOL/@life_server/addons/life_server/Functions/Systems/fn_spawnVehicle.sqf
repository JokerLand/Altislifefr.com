...
_query = format["SELECT id, side, classname, type, pid, alive, active, plate, color FROM vehicles WHERE id='%1' AND pid='%2' LIMIT 1",_vid,_pid];
...
_query = format["UPDATE vehicles SET active='1' WHERE pid='%1' AND id='%2' LIMIT 1",_pid,_vid];
...
if(typeName _sp == "STRING") then {
	...
	_vehicle setPosATL (_hs modelToWorld [-0.4,-4,15]);
	sleep 1;
...
_vehicle setVariable ["canInsure",true,true];
...
if((_vInfo select 1) == "cop" && (_vInfo select 2) in ["C_Offroad_01_F","B_MRAP_01_F","DAR_SWATPolice","ALE_GendOffroad","ALE_GendSUV","cl3_suv_black","ford_mondeo_gend","C_SUV_01_F","ALE_GendQ7"]) then
{
...
[[1,"Véhicule prêt!"],"life_fnc_broadcast",_unit,false] spawn life_fnc_MP;
serv_sv_use = serv_sv_use - [_vid];
