...
if(_houseID == -1) then {
	...
	_query = format["UPDATE houses SET owned='0', pos='[]' WHERE pid='%1' AND pos='%2' AND owned='1' LIMIT 1",_ownerID,_housePos];
	...
	_query = format["UPDATE houses SET owned='0', pos='[]' WHERE id='%1' LIMIT 1",_houseID];
};
