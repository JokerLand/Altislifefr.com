...
switch (_mode) do {
	case 0: {
		...
		_query = format["UPDATE gangs SET bank='%1', maxmembers='%2', owner='%3' WHERE id='%4' LIMIT 1",_bank,_maxMembers,_owner,_groupID];
	};
	
	case 1: {
		_query = format["UPDATE gangs SET bank='%1' WHERE id='%2' LIMIT 1",([(_group getVariable ["gang_bank",0])] call DB_fnc_numberSafe),_groupID];
	};
	
	case 2: {
		_query = format["UPDATE gangs SET maxmembers='%1' WHERE id='%2' LIMIT 1",(_group getVariable ["gang_maxMembers",8]),_groupID];
	};
	
	case 3: {
		_owner = _group getVariable["gang_owner",""];
		if(_owner == "") exitWith {};
		_query = format["UPDATE gangs SET owner='%1' WHERE id='%2' LIMIT 1",_owner,_groupID];
	};
	...
	case 4: {
		...
		_query = format["UPDATE gangs SET members='%1' WHERE id='%2' LIMIT 1",_membersFinal,_groupID];
	};
};
