...
if (_containerID isEqualTo -1) then {
    ...
    _query = format["UPDATE containers SET owned='0', pos='[]' WHERE pid='%1' AND pos='%2' AND owned='1' AND map='2'",_ownerID,_containerPos];
} else {
    _query = format["UPDATE containers SET owned='0', pos='[]' WHERE id='%1' AND map='2'",_containerID];
};
