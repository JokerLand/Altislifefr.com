...
for [{_x=0},{_x<=_count},{_x=_x+10}] do {
    _query = format["SELECT houses.id, houses.pid, houses.pos, players.name FROM houses INNER JOIN players WHERE houses.owned='1' AND houses.pid = players.playerid AND houses.map='2' LIMIT %1,10",_x];
