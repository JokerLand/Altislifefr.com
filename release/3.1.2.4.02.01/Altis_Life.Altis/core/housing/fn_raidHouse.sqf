...
if(!([_uid] call life_fnc_isUIDActive)) exitWith {hint "Cette personne n'est pas online vous ne pouvez pas raid!"};
_houseInv = _house getVariable ["Trunk",[[],0]];
if(_houseInv isEqualTo [[],0]) exitWith {hint "La maison est vide."};
...
5 cutText ["","PLAIN"];
if(player distance _house > 13) exitWith {life_action_inUse = false; titleText["Vous êtes trop éloigné!","PLAIN"]};
...
if(_value > 0) then {
	[[0,format["Une maison a été raid et $%1 de drogue/contrabande ont été saisi.",[_value] call life_fnc_numberText]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	...
	hint "Rien d'illégal dans la maison.";
};